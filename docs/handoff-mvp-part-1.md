...
| Move the support address to `support@influencertrees.com` | **After AWS answers the case**, not during the review, so the pages match the reply. Needs mail hosting for the apex domain first: the provider's MX, SPF and DKIM records go in the prod zone (`infra/prod`, Terraform), alongside SES's own DKIM and the DMARC record, which stay. Then: the app module's `contact_email` default, `bounce_notification_email` in each root (a new confirmation click), the directive line in `CLAUDE.md` and the design doc, and prod's SES request names the new address. Keep the pm.me inbox reachable for a while: mail already sent carries it as reply-to. The automatic stop address, if built, goes on a subdomain so the apex's mail records stay the provider's |
