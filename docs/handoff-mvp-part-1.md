...
| Confirm the bounce and complaint subscription | **Pending the click.** `bounce_notification_email` is set for beta, so the topic, its policy, the SES event destination and an email subscription to `influencertrees-support@pm.me` exist; that inbox must click *Confirm subscription* in the email from AWS Notifications, or nothing is delivered. Check: `AWS_PROFILE=iad-tf-beta aws sns list-subscriptions` shows `PendingConfirmation` until then. Prod repeats the bootstrap step before its own request |
